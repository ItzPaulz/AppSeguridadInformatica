...
const MiniGamePassword = ({ onComplete }) => {
  ...
  const checkPassword = () => {
    ...
    if (
      ...
      setMessage(
        "¡Contraseña SÚPER Segura! ¡Felicidades, eres un guardián de secretos!"
      );
      setIsComplete(true);
  ...
  return (
    <div className="p-6 bg-yellow-50 rounded-xl shadow-md text-center border border-yellow-200">
      <h3 className="text-xl font-bold text-yellow-800 mb-4">
        Crea una Contraseña Secreta
      </h3>
      <p className="text-yellow-700 mb-4">
        ¡Ayuda al detective a crear una contraseña muy fuerte!
      </p>
      <input
        type="text"
        value={password}
      ...
      <button
        ...
        className={`mt-4 py-2 px-6 rounded-lg shadow-md ${
          isComplete
